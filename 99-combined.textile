...
# interop: how to use Scala from Java
...
<pre>
class EvenMoreScientificCalculator(brand: String) extends Calculator(brand) {
...
h3. Case study: Thrust

See: https://github.com/twitter/thrust

<pre>
trait Service[-R <: Request] extends ((R, Response) => Unit)

trait Filter[-R1 <: Request, +R2 <: Request] {
  def apply(request: R1, response: Response, service: Service[R2])

  def andThen(that: Service[R2]) = new Service[R1] {
    def apply(request: R1, response: Response) {
      Filter.this(request, response, that)
    }
  }

  def andThen[R3 <: Request](that: Filter[R2, R3]) = new Filter[R1, R3] {
    def apply(request: R1, response: Response, service: Service[R3]) =
      Filter.this(request, response, new Service[R2] {
        def apply(req: R2, res: Response) = that(req, res, service)
      })
  }
}
</pre>

In Woodstar, credentials are added to requests as follows:
...
class CredentialsFilter(credentialsParser: CredentialsParser)
  extends Filter[Request, RequestWithCredentials]
{
  def apply(request: Request, response: Response, service: Service[RequestWithCredentials]) {
    val requestWithCredentials = new RequestWrapper with RequestWithCredentials {
      val underlying = request
      val credentials = credentialsParser(request) getOrElse NullCredentials
    }

    service(requestWithCredentials, response)
  }
}
</pre>

These can now be composed:
...
scala> res52 += (4 -> 1)
res56: res52.type = Map((2,3), (4,1), (1,2))
</pre>
...
is true, and a mess for when it is and isn't true.
