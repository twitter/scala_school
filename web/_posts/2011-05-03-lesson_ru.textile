...
** "А как же Карта?":#vsMap

h1. Основные структуры данных

В Scala существует несколько прекрасных коллекций.

*Смотрите также* В Effective Scalaописывается как использовать <a href="http://twitter.github.com/effectivescala/index-ru.html#Коллекции">коллеккции</a>.
...
В отличие от case классов, они у них нет именованных функций доступа, вместо этого у них есть функции доступа, которые носят названия по номеру позиции элемента кортежа и их базис равен 1, а не 0.
...
Map() тоже использует синтаксис, который мы изучили ранее на Уроке №1: <code>Map(1 -> "one", 2 -> "two")</code>, который раскрывается в <code>Map((1, "one"), (2, "two"))</code>, где первый элемент является ключом, а вторым элементом является некое значение Карты.
...
<code>Опция</code> представляет собой контейнер, который хранит какое-то значение или не хранит ничего.
...
Опция сама по себе это обобщенный тип и имеет два подкласса: <code>Some[T]</code> или <code>None</code>

Давайте взглянем на пример того, как Опция может использоваться:

<code>Map.get</code> использует <code>Option</code> для возврата собственных значений. Опция говорит вам, что метод может не вернуть того значения, которое вы запросили.
...
Первое, что приходит на ум, это сделать что-нибудь условное основываясь на методе <code>isDefined</code>.

<code>
// Мы хотим умножить число на 2, либо возвратить 0.
...
Мы предполагаем, что вы будете использовать либо <code>getOrElse</code> либо сопоставление с образцом для полученного результата.

<code>getOrElse</code> позволяет вам легкий способ объявить стандартное значение.

<code>
val result = res1.getOrElse(0) * 2
</code>

<code>Option</code> прекрасно вписывается при использовании сопоставления с образцом.
...
Наиболее распространенным является использование их со стандартными структурами данных.
...
foreach похож на map, но ничего не возвращает. foreach используется для оказания побочных эффектов.
...
zip объединяет содержимое двух списков в один список пар.
...
<code>drop</code> выбрасывает первый элемент с индексом i
...
<code>dropWhile</code> удаляет первый элемент, который не удовлетворяет функции-предикату. Например, если мы применим <code>dropWhile</code> к нечетным числам из нашего списка чисел, то <code>1</code> будет удалена (но не <code>3</code>, которая стоит за <code>2</code>).
...
0 - это начальное значение (Не забывайте, что мы используем числа из List[Int]), и m
...
Думайте об этом, как о краком способе использования map, а затем применения flatten:
...
Чтобы нам понадобилось, чтобы иметь возможность писать свои функциональные комбинаторы.
...
теперь отберем каждую запись, у которой телефонный код меньше 200.
