...
layout: post
desc: Еще больше функций! Частичные функции, еще больше сопоставления с образцом
---
...
<code>compose</code> дсоздает новую функцию, которая является объединением других функций <code>f(g(x))</code>
...
__Заметьте__ <code>PartialFunction</code> не связана с частично применяемыми функциями, о которых мы говорили ранее.
...
PartialFunctions может быть объединена с чем-то новым, называемым OrElse, что показывает, определен ли PartialFunction с передаваемым аргументом.
