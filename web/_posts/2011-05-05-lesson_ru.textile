...
layout: post
desc: Основные типы и полиморфизм типов, вывод типов, вариации, пределы, квантификация
---
...
По словам Пирса: "Система типов - это синтаксический метод для автоматической проверки отсутствия определенного ошибочного поведения, путем классификации фраз программы по видам значений, которые они вычисляют".

Типы позволяют определить область определения функции и ее множество значений. Например, из математике, мы привыкли видеть:
...
Абстрактно, это означает, что существует _конкретный_ тип. Система типов дает нам больше возможностей для выражения этих множеств.
...
Вообще говоря, проверка типов может лишь гарантировать, что _некорректные_ программы не будут скомпилированы. Она не может гарантировать, что каждый вариант программы _будет_ скомпилирован.

С увеличением выразительности в системе типов, мы можем писать более надежный код, потому что это позволяет нам доказать инварианты о нашей программе еще ​​до того как она заработает (по ошибкам в самих типах в модуле, конечно!). Научное сообщество распространяет границы выразительности очень тяжело, в том числе типов зависящих от значений!
...
* *(локальный) вывод типа*, проще говоря, почему вам не требуется писать так: <code>val i: Int = 12: Int</code>
* *квантор существования*, проще говоря, определяющих что-то _для некоторого_ неименованного типа
* *виды* о них мы узнаем на следующей неделе, проще говоря, "преобразование" значений одного типа в другой.
...
Например, без параметрического полиморфизма, общий список структуры данных всегда будет выглядеть следующим образом (и это действительно выглядело так в Java до введения дженериков):
...
Проще говоря, это означает, что существуют некоторые концепции типов, которые вы бы захотели использовать в Scala, и которые являются "слишком общими" для компилятора, чтобы их понять. Предположим, у вас есть некоторые функции
...
Классический метод для вывода типов в функциональных языках программирования _Хиндли-Милнер_, который был впервые применен в ML.

Система вывода типов в Scala работает немного по-другому, но близка по духу: выводить ограничения и попытаться определить тип.
...
Сейчас типы определены, Scala компилятор вывел типовый параметра для нас. Отметим также, что нам не нужно указывать тип возвращаемого значения явно.
...
Связь подтипа на самом деле означает: для данного типа T, если T' является подтипом, можете ли вы заменить его?
...
контрвариант выглядит странным. Когда он используется? Немного сюрпризов!

<pre>
trait Function1 [-T1, +R] extends AnyRef
</pre>

Если вы думаете об этом с точки зрения замещения, в этом есть смысла. Давайте сначала определим простую иерархию классов:
...
Напомним, что аргументы метода являются контравариантными, и поэтому, если мы выполняем нашу замену, используя теже классы, как и раньше:
