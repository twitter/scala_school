...
layout: post
desc: Дополнительные типы, видимое ограничение, типы высшего порядка, рекурсивные типы, структурные типы
---
...
*Неявные* функции в Scala позволяют использовать приложению функции по требованию, когда это может помочь при выводе типа, например:
...
Это говорит, что *A* должен быть "видимым" как *Int*.  Давайте попробуем.
...
В Scala 2.8 введена сокращенная для передачи и доступа с использованием неявных аргументов.
...
То есть мы должны удовлетворять определенному "уровню" типовых переменных с целью получения конкретных типов (подобно тому, как uncurried функция должна применяться к только одному списку аргументов, при вызове), типам высшего порядка требуется больше:
...
Часто, если необходим доступ к конкретному подклассу в (обобщенном) трейте. Напрмер, представьте себе некоторый трейт, который является обобщенным, но может быть сравним с конкретным подклассом данного трейта.
...
не скомпилируется, так как мы определяем Ordered для *Container*, а не конкретный подтип.
...
Обратите внимание, как результирующий тип в настоящее время ограничен снизу *YourContainer с MyContainer*. Это работа вывода типов. Интересно, что этому типу не требуется дополнительного смысла, он только обеспечивает логическую нижнюю границу для единого типа списка. Что произойдет, если мы попытаемся использовать *Ordered* сейчас?
...
*Ordered[]* не существует для единого типа. Слишком плохо.

h3. Структурные типы

Scala имеет поддержку *структурных типов* -- необходимость типа выражается интерфейсом _structure_ вместо конкретного типа.
...
Это часто полезный трюк, когда делается внедрение зависимостей, например.
...
h3. Тип чисток и манифесты

Как вы знаете, информация о типе теряется во время компиляции благодаря _erasure_. Одна из особенностей Scala - *Manifests*, позволяющая выборочно восстановить информацию о типе. Манифесты предоставляются в качестве неявного значения, генерируемый компилятором по мере необходимости.
