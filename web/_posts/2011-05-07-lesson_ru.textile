...
* будьте уверены, что является исполняемым и находится в указанном вами пути
...
* src/main - код вашего приложения находится здесь, во вложенной директории, которая называется по имени используемого языка (например, src/main/scala, src/main/java)
* src/main/resources - набор статических файлов, которые вы отите добавить к вашему jar-файлу
...
SBT может использоваться как скрипт командной строки или как консоль для сборки. Мы будем использовать его в основном как консоль для сборки, но большинство сомманд могут быть исопльзованы как аргументы при передаче их в качестве аргумента в SBT, например
...
Наш код скомпилировался, и попадаем в обычный Scala REPL. Мы создадим новый парсер, тестовый твит, и убедимся, что все "работает"
...
Вы объявляете зависимости с помощью val. SBT использует отражение, чтобы найти все val зависимости в вашем проекте и построить дерево зависимостей во время сборки. Синтаксис может показаться новым, но он эквиваленте Maven
...
Вы увидите, что sbt загрузит все необходимые библиотеки. Теперь у вас появилась директория lib_managed, а lib_managed/scala_2.8.1/test будет содержать specs_2.8.0-1.6.5.jar
...
Теперьдавайте перепишем наш JSON парсер, чтобы он больше соответсвовал действительности
...
и (спустя некоторое время), мы зайти на binaries.local.twitter.com:http://binaries.local.twitter.com/maven/com/twitter/sample/1.0-SNAPSHOT/ и увидеть наш опубликованный jar файл.
...
Итак, все работает. Если объявляете задачу в один проект, то все работает прекрасно. Но если вы определяете задачу как плагин, то это будет непрактично. Я хочу, чтобы 
