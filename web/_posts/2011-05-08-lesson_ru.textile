...
Scala предоставляет прекрасный набор коллекций. Также доступно некоторое количество абстракций для типов коллекций. Это позволяет вам писать код, который может работать с коллекцией <code>Foo</code> без необходимости беспокоится о том является коллекция <code>List</code>, <code>Set</code>, или что-то еще.
...
* "Основы":#basics Collection types you'll use all the time
* "Иерархия":#hierarchy Collection abstractions
* "Методы":#methods
* "Mutable":#mutable
* "Java коллекции":#java just work
...
Все коллекции имеют возможность обхода. Этот трейт определят стандарт функциональных комбинаторов. Эти комбинаторы написаны в терминах @foreach@, который коллекции могут реализовать.
...
Все методы ниже доступны для обхода. Аргументы и возвращаемые типы не всегда будет выглядеть так, потому что подклассы могут быть свободно переопределены.
...
Похоже на Java. Вы не будет часто видеть итераторы в программе на Scala, у вас больше шансов увидеть функциональные комбинаторы или for-выражения.
...
*TreeMap* - это подкласс SortedMap, дает вам упорядоченный доступ. "TreeMap API":http://www.scala-lang.org/api/current/scala/collection/immutable/TreeMap.html
...
*Range* Упорядоченная последовательность целых чисел, отделенных с определенным шагом. Вы будете часто видеть их там, где до этого использовались циклы. "Range API":http://www.scala-lang.org/api/current/scala/collection/immutable/Range.html
...
Используя методы apply в трейтах, вы получаете экземпляр стандартной реализации. Например, Iterable(1, 2) возвратит List как стандартну реализацию метода apply.
...
Мы начинаем с immutable версий коллекций, но переход на mutable, если необходима производительность. Использование immutable коллекций означает, что вы случайно не измените состояние в нескольких потоках.
