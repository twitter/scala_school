...
@doFirst@/@doLast@ предназначены во время установки (нужен пример, я не использую их)
...
Записиь короче чем try catch с его попыткой выброса исключения в теле блока.
...
Использования case класс вы делаете его менее специализированным.
