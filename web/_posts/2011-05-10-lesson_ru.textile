...
Когда вы видите класс реализующий Runnable, знайте, что он предназначен для запуска в Thread кем-то для выполнения какой-то работы.
...
Ниже представлен наш старый блокирующий сетевой сервер, позовляющий исопльзовать параллельные запросы.
...
В модели памяти Java, каждый процессор имеет право кэшировать значения в L1 и L2 кэш, два потока работающих на разных процессорах, могут иметь свои собственные виды данных.
...
@volatile@ и @synchronized@ строятся поверх встроенных мониторах в Java. Мониторы стоят очень мало, если нет никаких разногласий. @synchronized@ позволяет более тонкий контроль над синхронизацией, будет меньше конкуренции, поэтому @synchronized@, как правило, самый дешевый вариант.

Когда вы входите в точки синхронизации, пытаетесь обратиться к изменчивым ссылкам или используете AtomicReferences, Java заставляет процессор очитстить кэш-память и обеспечивает согласованное представление данных.

ПОЖАЛУЙСТА, ПОПРАВЬТЕ МЕНЯ, ЕСЛИ Я ЗДЕСЬ ОШИБАЮСЬ. Это сложная тема, и я уверен, что будут продолжительной дискуссии на этот счет.
...
@ReadWriteLock@ позволяет вам блокировать потоки читатели и писатели.  Читатель блокируется, только когда писатель установил блокировку.
...
Мы может смешать синронизацию с изменяющийся HashMap, используя трейт SynchronizedMap.
