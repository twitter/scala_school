...
Filters compose together with <code>andThen</code>. Providing a <code>Service</code> as an argument to <code>andThen</code> creates a (filtered) <code>Service</code> (types are provided for illustration).
...
val filter2 = 
  filter andThen filter1

val service = 
  filter2 andThen service
</pre>

h2. Builders

Finally, builders put it all together.
