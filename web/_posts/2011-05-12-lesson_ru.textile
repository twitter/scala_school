...
Finagle использует <code>com.twitter.util.Future</code>[1], чтобы описать отложенные операции. Futures очень выразительные и компактные, позволяющие кратко описать параллельные и последовательные операции с большой ясностью. Futures управляют значениями, которые еще не доступны, с методами для регистрации обратного вызова, который вызывается, когда значение становится доступным. Они переворачивают с ног на голову "традиционную" модель асинхронных вычислений, которые обычно предоставляют API, похожее на это:
...
Futures сами по себе это комбинаторы, с которыми мы столкнулись в различных API коллекций. Комбинаторы работают, используя единый API, оборачивая некоторые <code>Future</code> новым поведением без изменения <code>Future</code>.
...
используем future <code>f: Future[(User, Boolean)]</code>, который предоставляет объект пользователь и логическое выражение, которое сигнализирует о достигнутости пользователем предела. Заметьте, как здесь требуется последовательная композиция: <code>rateLimit</code> берет аргумент выходного результата <code>authenticate</code>
...
<code>join</code> берет последовательнсоть <code>Future</code> типы которых можно смешать, возвращая <code>Future[Unit]</code>, который завершится, когда все остальные futures будут завершены (или завеешится с ошибкой, если в одном из ни ошибка). Это полезно для отслеживания завершения набора гетерогенных операции.
...
В совокупности, это позволяет мощное и краткое выражение основных сетевых операций. Этот гипотетический код выполняет установку ограничений (с целью созранения локального кеша) параллелльно с управлением запросами пользователя на серверной стороне:
...
Этот гипотетический пример объединяет последовательную и параллельную композиции. Также обратите внимание, что нет явного обработчика ошибок, только конвертирвоание ограничения отвечает за выброс исключения. Если future выдает здесь ошибку, она автоматически распространяется до возвращаемого <code>Future</code>.

h2. Service

<code>Service</code> это фукнция <code>Req => Future[Rep]</code> для запросов и типовых ответов. <code>Service</code> используется как на клиенте, так и на сервере: серверы реализуют <code>Service</code>, а клиенты используют сборщики, для создания одного из них для запросов.
...
Здесь показан способ как вы можете написать фильтр, который механизм задержки service.
