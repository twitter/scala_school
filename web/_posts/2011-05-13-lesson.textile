...
In the first window, start sbt in interactive mode (run @sbt@ from the command line) and then build and run the project from within sbt. This runs the @main@ routine in @Main.scala@. 
...
Now we're going to instantiate and run a client from the @sbt console@ (the console starts the Scala interpreter with a classpath that includes the compiled Searchbird sources, among other things). The console instantiates a @SearchbirdConsoleClient@ instance (from @SearchbirdConsoleClient.scala@). Start the console in the other window:
...
The @SearchbirdServiceConfig@ class specifies such a class. It specifies configuration parameters together with their defaults. (Any references to a "tracer" in this tutorial/code refer to the "Zipkin":https://github.com/twitter/zipkin distributed tracing system, which is not covered here and is not important in this tutorial.) 
...
*_HELP is the default value Nil or None?_*
...
*_HELP Recall that if the call returns a @Future@, we have to use a blocking @get()@ call to resolve the value contained within that future. (is this correct?)_*
