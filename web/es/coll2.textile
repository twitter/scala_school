...
prev: sbt.textile
next: specs.textile
title: Más coleciones
layout: post
---

Scala ofrece un buen conjunto de implementaciones de colecciones. También proporciona algunas abstracciones de tipos de colección. Esto le permite escribir código que puede trabajar con una colección de <code>Foo</code>s, sin preocuparse de si esa colección es un <code>List</code>s , <code>Set</code>s, o lo que usted quiera.


"Esta página":http://www.decodified.com/scala/collections-api.xml ofrece una gran manera de seguir las implementaciones	predeterminadas y los links para toda la documentación de Scala.

* "Basicos":#basics Tipo de colecciones que vamos a usar todo el tiempo.
* "Jerarquía":#hierarchy Abstración de colecciones.
* "Métodos":#methods
* "Mutable":#mutable
* "Colecciones Java":#java Solo trabaja.

h2(#basics). Lo básico

h3. List

La Lista ligada estándar.
...
Puedes usar cons como es de esperar en un lenguaje funcional.
...
Las secuencias no tienen orden definido.
...
(Nota que Seq devuelve una lista. <code>Seq</code> es un rasgo (trait); List es una hermosa implementación de Seq. Hay una fabrica de objetos <code>Seq</code> que, asi como los vez aquí, crean listas.)

*Véase también* "API doc":http://www.scala-lang.org/api/current/scala/collection/Seq.html

h3. Map

Maps son contenedores de claves de valor.	
