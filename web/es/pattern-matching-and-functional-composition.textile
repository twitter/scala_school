...
h3. Instrucciones cases

h4. Entonces ¿que son las instrucciones case ?
...
Una función trabaja para cada argumento de un tipo definido.En otras palabras,  es una función definida como <code>(Int) => String</code> toma algun entero (Int) y devuelve una cadena (String).

Una Función Parcial esta solo definida por ciertos valores de un tipo definido. Una Función Parcial(PartialFunction) <code>(Int) => String</code> podría no aceptar cualquier entero (Int)

<code>isDefinedAt</code> Es un método en <code>PartialFunction</code> que puede ser usado para determinar si la <code>PartialFunction</code> aceptara un argumento recibido.

__Nota__ <code>PartialFunction</code> no esta realacionada a una función parcialmente aplicada de la que hablaremos más adelante.
...
<code>PartialFunctions</code> puede estar compuesta con algo nuevo, llamado <code>orElse</code>, que refleja si la <code>PartialFunction</code> esta definida sobre el argumento proporcionado.
...
toma una función de filtro. En esta caso una  función predicado de <code>(PhoneExt) => Boolean</code>.


¡Un <code>PartialFunction</code> es un subtipo de Función a modo de que pueda filtrar y tomar un <code>PartialFunction</code>!
