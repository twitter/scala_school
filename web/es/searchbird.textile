...
Exploremos primero el proyecto por defecto que @scala-bootstrapper@ crea por nosotros. El proyecto está pensado como una plantilla. Terminaremos sustituyendo la mayoría de él, pero sirve como una base conveniente. Define un almacenamiento tipo llave-valo simple (pero completo). Incluye Configuración, una interfaz de thrift, exportación de estadísticas y registro.
...
La implementación aún tiene un detalle que se queda como ejercicio al lector: cuando sobreescribimos una llave con un nuevo documento, no removemos ninguna referencia al documento anterior el el índice en reversa.
