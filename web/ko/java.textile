...
* 변수에는 <name>_$eq 라는 메소드가 만들어진다. 다음과 같이 호출해 새 값을 할당할 수 있다.
(역주: 유추해보면 변수도 <name>()이라는 메소드를 통해 값을 읽을 수 있을 것이다. 글을 초벌번역하는 지금 시점에서는 따로 테스트를 해보지 않아 뭐라 말하긴 어렵지만 당연한 일이라 생각된다.)
...
값이나 변수에 @BeanProperty 애노테이션을 할 수 있다. 그러면 POJO 게터/세터 정의와 마찬가지로 게터와 세터를 만들어준다. isFoo와 같은 게터/세터를 원하면 BooleanBeanProperty 애노테이션을 붙이도록 하라. 그러면 보기 싫은 foo$_eq가 다음과 같이 된다.
...
자바와 함께 동작하기 위해 사용할 수 있는 스칼라의 애노테이션 목록이 http://www.scala-lang.org/node/106 에 있다.
...
이 트레잇에는 추상 메소드가 하나(traitName), 구현된 메소드가 하나(upperTraitName) 있다. 스칼라가 만들어내는 것은 무엇일까? MyTrait이라는 이름의 인터페이스와 MyTrait$class라 불리는 도우미 구현 클래스를 만든다.
...
단지 스칼라가 만들어 놓은 도우미 구현 클래스에 위임하면 끝난다. 원한다면 이를 오버라이드할 수도 있다.
...
스칼라 객체는 끝에 "$"가 붙은 클래스로 컴파일된다. 클래스와 도우미 객체를 만들어 보자.
...
스칼라 2.8에서는 객체를 다루는 것이 훨씬 쉬워졌다. 도우미 객체와 함께 정의된 클래스를 다루는 경우 2.8 컴파일러는 도우미 객체로 전달하기 위한 메소드도 생성해준다. 따라서 스칼라 2.8로 빌드한 경우, TraitImpl에 있는 메소드를 다음과 같이도 호출 가능하다.
