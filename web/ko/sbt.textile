...
* 건전한(?) 의존성 관리
** 의존성 관리에 Ivy를 사용
** 요청이 올때 만 업데데이트(Only-update-on-request) 모델
...
프로젝트를 SNAPSHOP 버전을 가지고 시작하는 것이 항상 좋은 습관이다.
...
필요하면 SBT에서 프로젝트가 의존하는 모든 모듈을 적재한 상태의 스칼라 REPL을 시작할 수 있다. 콘솔을 시작하기 전에 프로젝트 코드를 컴파일할 것이다.  이 REPL을 사용하면 쉽게 우리가 작성한 파서를 들여다 보면서 테스트할 수 있다.
...
코드가 컴파일되고, 익숙한 스칼라 프럼프트가 표시된다. 새 파서와 예제 트윗을 만들고, 파서가 잘 "동작"하나 보자.
...
방금 입력한 데이터를 대상으로는 파서가 잘 동작하지만, 테스트를 더 추가해 오류가 나는 것을 보고 싶을 것이다. 먼저 할 일은 specs 테스트 라이브러리와 진짜 JSON 파서를 프로젝트에 추가하는 것이다. 이를 위해서는 기본 SBT 프로젝트 구조를 벗어나 새 프로젝트를 만들어야 한다.
...
프로젝트 정의는 바로 SBT 크래스이다. 위 예에서는 SBT의 DefaultProject를 확장했다.
