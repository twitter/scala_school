...
Прежде чем мы вглянем на код, давайте запустим клиент и сервер, чтобы увидеть, как все работает. В��т то, что мы делаем:
...
Теперь давайте запустим стандартный сервис и клиент, к��торый подключается к этому сервису, и исследуем их через этот интерфейс. Откройте два окна, одно для сервера и одно для клиента.
...
Вставьте следующее оп��саний лекции в консоль:
...
На одной машине, наши простой поисковый движок, находящийся в памяти, не сможет искать фрагменты больше, чем размер оперативной памяти. Мы будем теперь п��таться исправить эту ситуацию, распределяя узлы с помощью простой схемы распределения (шардинга). Вот блок-схема:
...
Сводный индекс работает с набором базовых экземпляров @Index@. Обратите внимание, что он не заботится, как они на самом деле реализованы. Этот тип композиции обеспечивает большую гибкость в построении различных схем запросов. Мы не определяем схему распределения, и сводный индекс не поддерживает операции @put@. Они выполняются непосредственно дочерними узлами. @get@ реализуется с помощью запросов все�� наших дочерних узлов и выбирается первый успешный результат. Если таковых нет, то выбрасывается исключение. Отметим, что поскольку сообщается отсутствие значения, выбрасывая исключение, мы @handle@ (управляем) этим с помощью @Future@, превращая любое исключение в занчение @None@. В реальной системе, мы бы, вероятно, имели соответствующие коды ошибок для отсутствующих значений, а не использовали исключения. Исключения являются удобными и целесообразными для создания прототипов, но в других случаях они не подходят. Для того, чтобы отличить реальное исключение и отсутствующее значение, мне нужно посмотреть само исключение. Скорее всего, это лучший способ для указания различий непосредственно в типе возвращаемого значения.
