...
* "Other Type Bounds":#otherboundsๆ
...
บางครั้งเราก็ไม่ได้อยากที่จะกำหนดว่าชนิดตัวแปรไหนมันจะเท่ากับอะไร เป็นลูกหรือเป็นแม่ของตัวไหน เราแค่อยากเปลี่ยนแปลงบางอย่างแบบปลอมๆ (เอ๊ะ! ยังไงรอดูกัน) ตัว view bound หรือจะเป็นมุมมองที่ถูกกำหนดขอบเขตเอาไว้เฉพาะเจาะจงนั้นสามารถ "ถูกมองเป็น" อย่างอื่นได้ มันเข้าใจง่ายมากสำหรับการดำเนินการที่เกี่ยวกับการ "อ่าน" บนออปเจ็กต์แต่มันไม่ได้เป็นแบบนั้นกับการเปลี่ยนแปลงข้อมูลของออปเจ็กต์

ฟังก์ชัน *Implicit* ยินยมให้เราสามารถเปลี่ยนแปลงได้อย่างอัตโนมัต มันทำให้เราสามารถที่จะกำหนดฟังก์ชันได้ตามความต้องการของแอพลิเคชันและยังช่วยให้เราสามารถที่จะสนใจกับ type inference เช่น:
...
View bounds, จะคล้ายๆกับชนิดตัวแปรของ bounds เช่นความต้องการให้มีฟังก์ชันอยู่ในชนิดของตัวแปรที่ได้รับมา เราสามารถเฉพาะเจ้าจง view bound โดยใช้ <code><%</code> :
...
การกำหนดแบบนี้หมายถึง *A* สามารถมองเป็น *Int* ได้ ทีนี้ลองดูนี่:
...
h2(#otherbounds). type bounds แบบอื่นๆ

เมธอตสามมารถบังคับให้ชนิดของ bound มัความซับซ้อนมากได้โดยผ่านพารามิเตอร์ของ implicit นี้ ยกตัวอย่าง <code>List</code> มันซับพอร์ด <code>sum</code> บนเนื้อหาที่เป็นตัวเลขแต่อย่างอื่นมันไม่ซับพอร์ต ปัดโถ่วเอ้ย! ชนิดตัวแปรที่เป็นตัวเลขของ Scala มันไม่ได้แชร์มาจากคลาสแม่เดียวกันซะหน่อย ทำให้พอเราจะ <code>T <: Number</code> มันก็ทำไม่ได้ดิ แทนที่จะทำแบบนั้นเพื่อให้มันทำงานได้ Scala มีไลบรารี่ math <a href="http://www.azavea.com/blogs/labs/2011/06/scalas-numeric-type-class-pt-1/"> ที่ implicit <code>Numeric[T]</code> สำหรับชนิดของตัวแปร T ที่เหมาะสม</a> ดังนั้นแล้ว <code>List</code> สามารถประกาศแบบนี้ได้:

<pre>
sum[B >: A](implicit num: Numeric[B]): B
</pre>

ถ้าเราร้องขอ <code>Lis(1,2).sum()</code> เราไม่จำเป็นต้องส่งพารามิเตอร์ _num_ ไปให้มัน มันจะ implicitly หรือดำเนินการโดยปริยายให้ แต่ถ้าเราเรียกมันโดย <code>List("whoop").sum()</code> มันจะบอกว่าไม่สามารเซ็ตตัว <code>num</code> ให้ได้

เมธอตอาจจะถามสำหรับบางประเภทที่เฉพาะเจาะจง
