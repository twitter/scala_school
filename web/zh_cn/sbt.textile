...
SBT是一个现代化的构建工具。虽然它是用Scala编写并提供了许多Scala的便利，单它是一个通用的构建工具。
...
* Sane(ish) dependency management 明智的依赖管理
** Ivy for dependency management 使用Ivy做依赖管理
** Only-update-on-request model “只在有需求时更新”的模型
* Full Scala language support for creating tasks 创建任务时全面的Scala支持
* Continuous command execution 连续执行命令
* Launch REPL in project context 在项目范围内启动REPL
...
* make sure it's executable and in your path 确保它是可执行的，并在您的path下
* run sbt to create your project 运行SBT来创建项目
...
h2. Project Layout 项目布局

* project - project definition files 项目 - 项目定义文件
** project/build/<yourproject>.scala - the main project definition file 主要项目定义文件
** project/build.properties - project, sbt and scala version definitions  项目，sbt和Scala版本定义
* src/main - your app code goes here, in a subdirectory indicating the 
code's language (e.g. src/main/scala, src/main/java) 您的应用程序代码出现在这里，在子目录表明代码的语言（如src/main/scala, src/main/java）
* src/main/resources - static files you want added to your jar
  (e.g. logging config) ，你想要添加到jar包中的静态文件（如日志配置）
* src/test - like src/main, but for tests 就像src/main，但是是对测试
* lib_managed - the jar files your project depends on. Populated by sbt update 你的项目依赖的jar文件。由SBT更新时填充
* target - the destination for generated stuff (e.g. generated thrift
  code, class files, jars) 生成物的目标路径（如自动生成的样板[复杂？TODO]代码，类文件，jar包）

h2. Adding Some Code 添加一些代码

我们将为简单的tweet消息创建一个简单的JSON解析器。并将代码加在这个文件中
...
h2. Testing in the Console 在控制台中的测试
...
SBT可以使用命令行脚本，也可以作为构建控制台。我们将主要利用它作为构建控制台，但大多数命令可以作为参数传递给SBT独立运行，如
...
argument path, e.g.
需要注意如果一个命令需要参数，你需要使用引号包括整个参数路径，例如
<pre>
...
SBT允许你启动一个Scala REPL并加载所有的项目
依赖。他会在启动控制台前编译你的项目源代码
，为我们提供一个快速测试解析器的工作台。
...
我们代码编译通过了，并提供了典型的Scala提示。我们将创建一个新的解析器，一个tweet以确保其“能工作”
...
我们简单的解析器对这个非常小的输入集工作正常，但我们要添加更多的测试并让它出错。第一步是在我们的项目中添加specs测试库和一个真正的JSON解析器。要做到这一点，我们必须超越默认的SBT项目布局来创建一个项目。

SBT认为project/build目录中的Scala文件是项目定义。添加以下内容到project/build/SampleProject.scala
...
一个项目的定义是一个SBT类。在上面例子中，我们扩展了SBT的DefaultProject。

这里是通过val声明依赖。 SBT使用反射来扫描项目中的所有val依赖，并在构建时建立依赖关系树。这里的语法可能是新的，但是和Maven依赖是相同的
...
现在我们可以下载我们的项目依赖。在命令行（不是sbt console）中运行sbt update
...
你会看到sbt检索specs库。你现在还有了一个lib_managed目录，并且在lib_managed/scala_2.8.1/test目录中包含 specs_2.8.0-1.6.5.jar

h2. Adding Tests 添加测试

现在，我们添加了测试库，把下面的代码写入src/test/scala/com/twitter/sample/SimpleParserSpec.scala文件
...
我们的测试通过了！现在，我们可以增加更多。 运行触发动作是SBT提供的好东西之一。在动作开始添加一个波浪线会启动一个循环，在源文件发生变化重新运行动作。让运行~test 并看看会发生什么。
...
我们保存文件后，SBT检测我们的变化，运行测试，并通知我们的解析器有问题
...
这是一个简单的Jackson解析器。当我们保存，SBT重新编译代码和运行测试。一切尽在掌握！
...
嗯哦。我们需要检查嵌套对象。让我们在令牌读取循环添加一些丑陋的守卫。TODO
...
h2. Packaging and Publishing 打包和发布

在这一点上，我们可以运行package命令来生成一个jar文件。然而，我们可能要与其他组分享我们的jar包。要做到这一点，我们将在StandardProject基础上构建，这给了我们一个良好的开端。
...
第一步是包含StandardProject为SBT插件。插件是一种为你的构建引进依赖的方式，注意不是为你的项目引入。这些依赖关系定义在project/plugins/Plugins.scala文件中。添加以下代码到Plugins.scala文件中。
...
请注意，我们已经指定了一个Maven仓库和一个依赖。这是因为这个标准项目库是由twitter托管的，不在SBT默认检查的仓库中。

我们也将更新项目定义来扩展StandardProject，包括SVN发布特质，和我们希望发布的仓库定义。改变SampleProject.scala
...
这样（一段时间后），就可以在 binaries.local.twitter.com:http://binaries.local.twitter.com/maven/com/twitter/sample/1.0-SNAPSHOT/  看到我们发布的jar包。

h2. Adding Tasks 添加任务

任务是Scala的函数。添加一个任务最简单的方法是，在您的项目定义中引入一个val定义的任务方法，如
...
所以它起作用了。如果你只是在一个项目定义一个任务的话，这工作得很好。但是，如果你定义的是一个插件则它很不灵活。我可能要
...
这可以让消费者覆盖任务本身，依赖和/或任务的描述，或行动本身。大多数SBT内建的行动都遵循这种模式。作为一个例子，我们可以通过修改内置打包任务来打印当前时间戳
...
有很多例子介绍了调整SBT默认的StandardProject，和如何添加自定义任务。

h2. Quick Reference 快速参考

h3. Common Commands 常用命令

* actions - show actions available for this project 显示这个项目包含的行动
* update - downloads dependencies 下载依赖
* compile - compiles source 编译源
* test - runs tests 运行测试
* package - creates a publishable jar file 创建一个jar文件发布
* publish-local - installs the built jar in your local ivy cache 安装在您当地的ivy缓存内置的jar
* publish - pushes your jar to a remote repo (if configured) 你的jar推到一个远程回购（如果配置了）

h3. Moar Commands MOAR命令

* test-failed - run any specs that failed 运行所有失败的规格
* test-quick - run any specs that failed and/or had dependencies updated 运行任何失败的和/或依赖更新的规格
* clean-cache - remove all sorts of sbt cached stuff.  Like clean for sbt 删除SBT缓存各种的东西。想sbt的clean命令
* clean-lib - remove everything in lib_managed 删除lib_managed下的一切

h3. Project Layout 项目布局
